...
VAR:integer :: 1   // try different variants of distance to see what works best
...
[fitHourly(t:TSerie) : float
  -> let a := makeHourly(t),
         ...
         n := float!(t.count),
         nFactor := n / ( 30.0 + n) in
      (printf("fitHourly : ~F% n", distance(a,t) / distance(b, t)),
       // nFactor * stdeviation(a.values))
...
[fitWeekly(t:TSerie) : float
  -> let a := makeWeekly(t),
         ...
         n := float!(t.count),
         nFactor := n / ( 30.0 + n) in
      (printf("fitWeekly : ~F% n", distance(a,t) / distance(b, t)),
       // nFactor * stdeviation(a.values))
       distance(a,t) / distance(b, t)) ]

// lift a weighted average - simply tries 3,6,10
LIST_AVERAGE :: list<integer>(6,10) // (3,6,10)
// (1,2,3,4,6,10) : worse ! better correlation but no prediction robustness
...
// fit is simple (bigger the better)
[fitAverage(t:TSerie) : float
  -> let a := makeAverage(t),
         b := makeConstant(t) in
      (distance(b,t) / distance(a, t)) ]
...
[controlSum(t:TSerie,n:integer) : Sum
...
[makeSumCorr(t:TSerie,liftMode:integer) : Sum
  -> let a :=  makeConstant(t),  // makeTrend(t) is already more precise and less robust
...
[makeThreshold(t:TSerie,liftMode:integer) : Threshold
  ->  let tk := pickTracker(t,liftMode), best_l := 0.0, best_v := 0.0 in
        ...
        Threshold(from = tk,
               delay = 0,
	          coeff = (if (corSerie(t,tk.value) > 0.0) 1.0 else -1.0) *
                           (stdev(t) / stdev(threshold(tk.value,best_l))),
              level = best_l)) ]
...
[fitControl(t:TSerie,i:integer) : float
  -> if (i = 2) fitHourly(t)
     ...
     else if (i = 5 | i = 5) fitCorrelation(t)
...
[atp(p:Profile,im:integer,m:integer, f:float)
  -> let mt := p.target,               // master time-serie / reference for time
         n := p.target.count, ns := (n / 2),     // debug (2 * n / 3),
         ...
         let ms := measure!(p,min(n,i + 1)) in    // tuple of values
           (//[5] <<~A>> ======== start a tp at time  ~A // i, dateF(p.target.dates[i]),
            ...
            let a := (if (im < 0) TT4 else liftAndOpt(P2,im,m)),
                fit := fitControl(P2.target,im),
                v := forecast(a,mt.dates[min(n,i + 1)]),
                d := distance3(a, P2.target),                  // important : evaluate with h3
...
[liftAndOpt(p:Profile,i:integer,n:integer) : TSTerm
  ->  let t := p.target, a := makeControl(t,i) in
       (for j in (1 .. n)
          (a := opt1(a,t,false,3)),
        a) ]
...
// run 17 experiments 
[irun(e:AExperiment)
  -> for i in (1 .. length(pb.fnames)) run(e,i) ]
...
[logResult(e:AExperiment) : void
  -> let p := fopen(Id(*where*) / "data" / "log" /+ string!(name(e)),"a") in
       (use_as_output(p),
        printf("//~S ~A:~A [~A] avgDelta, avgDist, avgVolatility, runtime, fitness\n",
